/* ==========================================================================
   DSU KnotSpot Database Engine (localStorage Wrapper) - UPGRADED
...
const SEED_USER = {
    ...
    languagesSpoken: ["Kannada", "English", "Hindi"],
    nativePlace: "Mysore"
};
...
// Available Rooms Seed Data (Listings posted by students offering a room/flat)
const SEED_ROOMS = [
    {
        id: "r-1",
        title: "Furnished room in 2 BHK flat near KS Layout",
        description: "Spacious bedroom available in a 2 BHK apartment near Kumaraswamy Layout gate. Flat is fully set up with sofa, dining, and smart TV. Looking for a friendly flatmate.",
        rent: 6000,
        deposit: 15000,
        campus: "ks-layout",
        roomType: "2bhk",
        location: "Vasanthapura Road, KS Layout",
        totalOccupants: 2,
        currentOccupants: 1,
        neededOccupants: 1,
        genderPreference: "boys",
        furnishingStatus: "semi",
        amenities: ["WiFi", "Washing Machine", "Attached Bathroom"],
        contact: "+91 98765 43210",
        authorName: "Sandeep Rao",
        authorAvatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80",
    ...
    {
        id: "r-2",
        title: "Sharing room at Green View PG near Kanakapura Campus",
        description: "Double sharing room available in a highly recommended student PG. Daily cleaning, hot water geyser, high-speed Wi-Fi, and 3-meals vegetarian food are included in the rent.",
        rent: 5200,
        deposit: 5000,
        campus: "kanakapura",
        roomType: "pg",
        location: "Harohalli Layout, Kanakapura Road",
        totalOccupants: 2,
        currentOccupants: 1,
        neededOccupants: 1,
...
// Room Requests Seed Data (Listings posted by students looking for roommate/flat)
const SEED_ROOM_REQUESTS = [
    {
        id: "req-1",
        title: "18F looking for flatmate/room near KS Layout DSU",
        campus: "ks-layout",
        preferredRoomType: "flatmate",
        lookingNear: "Kumaraswamy Layout / Banashankari",
        minRent: 4000,
        maxRent: 7000,
        sharingPreference: "2 sharing",
        moveInDate: "2026-08-01",
        neededAmenities: ["WiFi", "Washing Machine"],
        extraNote: "Looking for neat and studious roommates, vegetarian preferred. Non-interfering vibe.",
        poster: {
            name: "Ananya Iyer",
    ...
    {
        id: "req-2",
        title: "20M CSE student looking for PG or room near Harohalli Campus",
        campus: "kanakapura",
        preferredRoomType: "pg",
        lookingNear: "Harohalli main road",
        minRent: 3500,
        maxRent: 5500,
        sharingPreference: "any",
        moveInDate: "2026-07-28",
        neededAmenities: ["WiFi", "Food", "AC"],
        extraNote: "Cool roommate, interested in coding and gaming. Okay with both veg/non-veg foods.",
        poster: {
            name: "Rahul Hegde",
            avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80",
            nativePlace: "Mangalore",
            motherTongue: "Tulu",
            languagesSpoken: ["Tulu", "Kannada", "English"]
        },
        createdAt: "2026-07-19T12:00:00Z"
    }
];
...
function initDatabase() {
    ...
    if (!localStorage.getItem('dsu_room_requests')) {
        localStorage.setItem('dsu_room_requests', JSON.stringify(SEED_ROOM_REQUESTS));
    }
...
const db = {
    ...
    saveUser(user) {
        // Carry forward native properties if editing
        const oldUser = this.getUser();
        const merged = { ...SEED_USER, ...oldUser, ...user };
        localStorage.setItem('dsu_user', JSON.stringify(merged));
        return merged;
    },
    logoutUser() {
    ...
    // Roommate Finder - Available Rooms (Rooms offered by students)
    getRooms() {
        return JSON.parse(localStorage.getItem('dsu_rooms'));
    },
    createRoomListing(title, description, rent, deposit, campus, roomType, location, totalOccupants, currentOccupants, genderPreference, furnishingStatus, amenities, contact, imageUrl) {
        ...
        const newRoom = {
            ...
            totalOccupants: Number(totalOccupants || 1),
            currentOccupants: Number(currentOccupants || 1),
            neededOccupants: Number(totalOccupants - currentOccupants) > 0 ? Number(totalOccupants - currentOccupants) : 1,
    ...
    // Roommate Finder - Room Requests (Posted by students looking for roommate/PG)
    getRoomRequests() {
        return JSON.parse(localStorage.getItem('dsu_room_requests'));
    },
    createRoomRequest(title, preferredRoomType, lookingNear, minRent, maxRent, sharingPreference, moveInDate, neededAmenities, extraNote) {
        const requests = this.getRoomRequests();
        const user = this.getUser();
        const newRequest = {
            id: "req-" + Date.now(),
            title: title,
            campus: user ? user.campus : "ks-layout",
            preferredRoomType: preferredRoomType || "any",
            lookingNear: lookingNear || "Near campus",
            minRent: Number(minRent || 0),
            maxRent: Number(maxRent || 10000),
            sharingPreference: sharingPreference || "any",
            moveInDate: moveInDate || new Date().toISOString().split('T')[0],
            neededAmenities: neededAmenities || [],
            extraNote: extraNote || "",
            poster: {
                ...
                nativePlace: user && user.nativePlace ? user.nativePlace : "Karnataka",
                motherTongue: user && user.motherTongue ? user.motherTongue : "Kannada",
                languagesSpoken: user && user.languagesSpoken ? user.languagesSpoken : ["Kannada", "English"]
            },
            createdAt: new Date().toISOString()
        };
        requests.unshift(newRequest);
        localStorage.setItem('dsu_room_requests', JSON.stringify(requests));
        return newRequest;
    },
